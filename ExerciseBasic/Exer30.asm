...
; Description: Display smaller and bigger of two integer

.model small
.stack 100h
.data
    prompt1             db 'Enter first integer: $'
    prompt2             db 13, 10, 'Enter second integer: $'

    buffer              db 6,?,6 dup('$')  ; Buffer for input (max 5 digits + sign)

    bigg                db 13, 10, 10, "Bigger: $"
    smol                db 13, 10, "Smaller: $"
...
main proc                  
    ...
    mov dx, num2   
    cmp ax, dx 
    jg isBigger                     ; if num1 > num2
    jl isSmaller                    ; if num1 < num2
